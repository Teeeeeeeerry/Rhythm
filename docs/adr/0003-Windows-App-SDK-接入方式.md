...
>
> 2026-09-19（#342）：行为代码不再使用任何 C++/WinRT（主题解析移到壳），配置期不再生成投影头，`Rhythm::CppWinRT` 随之删除；CMake 只导出 `nlohmann_json::nlohmann_json`，C++/WinRT 工具链与 Windows App SDK 只经 props 交给 MSBuild 应用。
