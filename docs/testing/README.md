# Rhythm 深度测试套件（L0–L4）

> 依据 [docs/deep-testing-plan.md](../deep-testing-plan.md) v2 实现。
> 目标：**一切可机检的皆自动化**，手工只留 8 项主观审美判断。

## 分层一览

| 层 | 目录 | 内容 | 运行时机 |
|---|---|---|---|
| 数据源 | `palette.json` | 单一事实来源（tokens/sources 由源码自动提取，决策段人工维护） | — |
| 数据源 | `sync-palette.py` | 源码 ↔ palette.json 同步；`--check` CI 校验；`--emit-swift-seed` 生成 L1 种子 | 改色后 |
| L0 静态 | `l0/` | 5 个零依赖 Python 脚本（parity/contrast/forbidden/coverage/doc-drift） | 每次 push/PR |
| L1 单元 | `l1/macos/` | PaletteSeed + 五组 Swift 测试（isDark/RGB/对比度/语义/互异） | `swift test` |
| L1 单元 | `l1/windows/` | 来源徽标色 assert 测试 exe（F1 修复后签名） | ctest |
| L2 快照 | `l2/macos/` | swift-snapshot-testing 模板（8 视图 × 状态 × 外观 × 语言） | visual CI |
| L2 快照 | `l2/windows/` | WinUI 3 离屏截屏（capture_views.cpp）+ 零依赖像素 diff | visual CI |
| L3 UI | `l3/macos/` | XcodeGen project.yml + 4 组 XCUITest（外观切换/键盘/a11y/新建弹窗） | visual CI |
| L3 UI | `l3/windows/` | WinAppDriver 兼容性验证 + 主题切换脚本（stdlib 直调 REST） | visual CI |
| L4 手工 | `l4/` | 8 项主观烟测清单（唯一人工环节） | PR 合并前 |
| CI | `ci/` | ci.yml（L0+L1）、visual.yml（L2/L3/Nightly）模板 | 拷贝部署 |

## 快速开始（本地全量）

```bash
cd /Users/home-folder/GitHub/Rhythm

# 1. 数据源同步与自检
python3 docs/testing/sync-palette.py --check        # palette.json 与源码一致？
python3 docs/testing/sync-palette.py --emit-swift-seed  # 刷新 L1 测试种子

# 2. L0 静态分析（预期现状：F1/F2/F4 三项红 —— 正是 P0 修复验收信号）
#    每个脚本结束自动把完整输出写入 docs/testing/logs/<脚本名>.log（--log 可覆盖）
python3 docs/testing/l0/check-color-parity.py
python3 docs/testing/l0/check-contrast.py
python3 docs/testing/l0/check-forbidden-colors.py
python3 docs/testing/l0/check-token-coverage.py
python3 docs/testing/l0/check-doc-drift.py
# 或一键全量（含 sync 校验 + L1 swift test，日志统一落盘）：
bash docs/testing/run-all.sh
# Windows 侧：
powershell -File docs/testing/run-windows.ps1 -Smoke

# 3. L1（P2 重构已完成：Theme.swift 拆为 RhythmTheme target）
python3 docs/testing/sync-palette.py --emit-swift-seed   # 刷新测试种子
cp docs/testing/l1/macos/*.swift macos/Tests/RhythmThemeTests/  # 拷贝挂载（与 CI 一致）
cd macos && swift build && swift test

# 4. L2 Windows 像素工具自测
python3 - <<'EOF'
import sys; sys.path.insert(0, "docs/testing/l2/windows")
from compare_screenshots import PNG
print("PNG 解码器可用")
EOF
```

## 当前状态（fix/43-macos-brand-colors 分支）

| 检查 | 现状 | 含义 |
|---|---|---|
| `check-color-parity.py` | 🔴 | **F1**：C++ source 色缺 light 变体（P0 修复项） |
| `check-contrast.py` | 🟢 | 36 组合全达标或已登记例外（F8 两项 + border 装饰线已登记） |
| `check-forbidden-colors.py` | 🔴 | **F4**：SourceTagView 回退 `.gray`（P0 修复项） |
| `check-token-coverage.py` | 🔴 | **F2**：Windows SidebarView 零品牌化（P0 修复项） |
| `check-doc-drift.py` | 🟢 | 文档色值全部收录于 palette.json |

L0 全绿 = P0（F1–F5）完成。合并门槛见 deep-testing-plan.md §7。

## 关键约定

1. **改色流程**：改 `Theme.swift` / `Colors.xaml` / `RhythmCore.h` →
   `sync-palette.py`（刷新 palette.json + 测试种子）→ L0 五脚本 → `swift test`。
   任何一步红都要解释，禁止 `|| true` 静默吞掉。
2. **palette.json 决策段**（usage/backgrounds/exceptions/whitelist）人工维护，
   是 L0 检查的"立法"：低对比度要么修复要么登记例外，两者都留痕。
3. **禁止裸色**是硬约束：视图代码只准出现 `.rhythm*` token；新视图必须有 token。
4. **快照维护**：外观改动必附 golden 更新，review 看 diff；CI 快照红 = 真回归。
5. **手工最小化**：合并前 L4 八项勾选（l4/manual-smoke-checklist.md），
   每项 ≤ 3 分钟。
6. **日志留痕**：任何测试结束后必有日志 —— Python 脚本自动把完整输出双写
   终端与 `docs/testing/logs/<脚本名>.log`（`--log` 可覆盖路径）；
   `swift test` / `ctest` / `xcodebuild` 由 `run-all.sh` / `run-windows.ps1`
   或 CI tee 落盘（xcodebuild 另有 `.xcresult` 结构化日志）。
   `docs/testing/logs/` 已 gitignore，CI 每次运行作为 artifact 上传。
7. **一键入口**：`bash docs/testing/run-all.sh`（macOS）、
   `powershell -File docs/testing/run-windows.ps1 -Smoke`（Windows）
   跑完本机支持的全部层级，日志齐后看 `docs/testing/logs/`。
