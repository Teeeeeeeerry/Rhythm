...
职责（供 docs/testing/l0/*.py 与 sync-palette.py 复用）：
...
def palette_path(repo_root: Path) -> Path:
    return repo_root / "docs" / "testing" / "palette.json"
...
def default_log_path(script_name: str, repo_root: Path | None = None) -> Path:
    """默认日志位置 docs/testing/logs/<脚本名>.log（每次运行覆盖）。"""
    root = repo_root or find_repo_root()
    return root / "docs" / "testing" / "logs" / f"{script_name}.log"
...
def open_log(path: Path) -> Path:
    ...
    结束后 docs/testing/logs/ 下必有完整输出可查。
    """
    path = Path(path)
