...
| WB-30 | 导出与删掉的手写编码器逐字节一致（#385） | 同一组曲目（本地全字段 + 缺艺人的 URL 曲目）分两次导出：一次用歌单详情原先手写编码器的载荷形状（每个键都在、空可选显式 null）直调核心，一次经 `ExportM3U8`（生成的编码器）；两份 M3U8 逐字节相同（含缺艺人回退 `Unknown Artist`）。曲目编码只剩生成物这一处出处 | 新测（真 core） |
