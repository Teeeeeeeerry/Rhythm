...
- 分派归核心（#216 组收尾，#324 延伸到导入结果）：三条分派（播放失败、解析失败、解析器状态）与四条导入路径的结果文案（目录、单文件、批量、M3U8，MS-09~12 / LK-14~17）全部下沉到 `rust-core/src/message/`，双端适配层只剩按键取模板与填占位符；新增分类只改核心与键表
