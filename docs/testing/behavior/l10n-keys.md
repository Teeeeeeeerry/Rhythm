...
- 校验：`testing/l0/check-l10n-keys.py`（#185）——键表结构、双端生成物一致性、Windows 映射覆盖，任一红即 run-all 非零退出；
  ...
  （检验面从键名表扩到调用面；自测 `testing/l0/tests/test_check_l10n_keys.py`）。本票落地时预期报红：`TrayQuit` 被调用却不存在，
  另有一批键无访问器，由 #371 的访问器生成转绿
...
| LK-10 | 播放模式提示接线（#373） | Windows 播放栏新增播放模式控件（点击循环 `CyclePlayMode`，图标取 `AppState.h` 的 `PlayModeIcon`：顺序 List / 随机 Shuffle / 单曲循环 RepeatOne / 列表循环 RepeatAll），提示文案取键表 `play_mode_tooltip`（生成访问器 `PlayModeTooltip()`）；中英两语言均能取到 | Windows 固定 locale（`L10nTests.cpp`：文案与四种模式图标）；点击循环后图标随模式（`AppStateBehaviorTests.cpp`，#411） |
