...
| LK-10 | 播放模式提示接线（#373） | Windows 播放栏新增播放模式控件（点击循环 `CyclePlayMode`，图标取 `AppState.h` 的 `PlayModeIcon`，返回不含 WinUI 类型的 `rhythm::Icon`：顺序 List / 随机 Shuffle / 单曲循环 RepeatOne / 列表循环 RepeatAll，由播放栏换成 XAML Symbol，#329），提示文案取键表 `play_mode_tooltip`（生成访问器 `PlayModeTooltip()`）；中英两语言均能取到 | Windows 固定 locale（`L10nTests.cpp`：文案与四种模式图标）；点击循环后图标随模式（`AppStateBehaviorTests.cpp`，#411） |
