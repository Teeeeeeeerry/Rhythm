...
- 模块：`rust-core/src/library/mod.rs`（SQLite 曲库：CRUD、去重、播放列表、FTS 搜索、目录/文件/批量导入）
- #244 起：导入路径上不再有魔数返回码——`import_from_directory` 与 `import_file` 已删除，三条路径只返回 `ImportOutcome`
...
| LB-15 | `import_directory` 递归扫描 | 递归入库；扫描不出的文件不中断整体；返回具名结果（#244 起魔数入口已删除，断言改为计数） |
| LB-16 | `import_single_file` | 支持格式 → `imported` = 1；metadata 与 artwork 提取后入库（#244 起魔数入口已删除） |
...
| LB-20 | `import_single_file` 不存在的文件 | `failed` = 1，不 panic，且不算作格式不支持 |
...
| LB-23 | 同目录重复 `import_directory` | file_path 去重（第二次导入不产生重复行） |
...
| LB-26 | ~~`import_from_directory` 非目录路径~~ | 已并入 LB-29（#244）：魔数入口删除后，非目录路径与路径不存在同为「扫描跑不起来 → `failed` = 1」 |
...
LB-24（open 不可写路径）、LB-25（record_play 不存在 id）已随 Wave 3a 覆盖
（`library_behavior.rs`），无需顺延；LB-26 自 #244 起并入 LB-29。
