...
> 播放编排条目（AS-02~15、AS-17/18、AS-22、AS-25 及边界 AS-27/28/34/35 的编排部分）
> 自 #175 起并入 `coordinator.md`（CO-01~28），双端不再按平台重复维护；本清单只保留
> AppState 的 UI 状态与流程（导入、搜索、删除确认、URL 解析导入）。
...
| AS-24 | `importDirectory`/`importFile` 单路径（#240） | 断言基于具名结果：`imported>0` → 成功文案 + 刷新；目录 `failed>0` → 目录失败文案，三项全 0 → 目录为空文案（目录三态分派下沉核心，见 l10n-keys.md LK-14/MS-09，#376）；单文件 `unsupported>0` → 格式不支持文案，`failed>0` → 读取失败文案 | 真库 + 夹具目录 |
...
| AS-41 | 播放失败信息可见 | Error 状态下 `urlError` 含 core 的错误详情 | SpyCoordinator（预置 error 事件） | 已覆盖（AS-15 已断言含 core 详情，见主路径表；此处仅登记出处） |
| AS-42 | 播放失败 HTTP 分类文案（#120） | `expired` → 保留"重新粘贴"建议；`cdn_rejected` → 换网络/稍后再试且**不再建议重贴**；其它 → 泛化"播放失败" | SpyCoordinator（error 事件携带 kind） | 已覆盖（`testUpdatePlaybackProgress_Error_ExpiredKind_KeepsRepasteAdvice`、`testUpdatePlaybackProgress_Error_CdnRejectedKind_BlamesNetwork`） |
