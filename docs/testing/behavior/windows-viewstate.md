...
| VS-38（#358） | 歌单详情 `PlaylistDetailOf(state, isDark)` | 输入只有编排状态与主题：有当前歌单 → `hasPlaylist` 为真、标题为歌单名、行按歌单内顺序；没有 → `hasPlaylist` 为假、标题与行都为空；刷新（导入 M3U8 会触发）后的内容在下一次渲染即可见。详情页因此不持有歌单指针或标识 | 新测 |
