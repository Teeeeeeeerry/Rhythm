...
- #363 起：协调器结果 `coordinator_result` 同样改为带类型的对象声明（`current_track` 引用曲目对象），`results` 段删除——契约里除版本、说明与枚举表外的每个条目都是生成的对象（生成器自测锁定）。Windows 新增 `CoordinatorResult` 编解码，模型的错误两项随契约改为可缺省；解码断言见 `rhythmcore-windows.md` WB-25/WB-26
