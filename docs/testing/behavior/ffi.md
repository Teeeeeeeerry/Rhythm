...
| CR-04 | 显式空值往返后只会变成缺省 | 每个可选字段写成 `null` 的载荷，解码结果与缺省时相同；再编码这些键缺席，不回写 null 也不变成默认值 | 新测；同上两种缺陷即红 |
| CR-05 | 整型可选字段的缺省形态单独断言 | 模型里的整型可选成员恰为契约声明的 `i32?`/`i64?` 字段；缺省与 `null` 都解码为空（不是 0）且编码时缺席；显式 0 是一个值：编码写出 0、解码读回 0，不被当成缺省。契约里至少有一个整型可选字段（曲目的七个），用例不会空跑 | 新测；编码器丢掉显式 0，只有本条红 |
