...
- #323 收口：接缝上原有三份曲目适配器（生成的那份编译不了、桥接层两段手写解码、歌单详情视图里一份类型正确的手写编码器），现在只剩生成的一份——生成器按字段类型分派（#360/#413），契约里声明的每个对象都在生成范围内（#362/#363/#367/#368），两份手写的删除（#364/#428），门从「文本一致」升级为「文本一致 + 生成物被编译 + 往返测试穿过接缝」（#365/#366/#369）。Swift 侧的生成范围同样取自契约：默认全生成，macOS 仍走自己 Codable 路径的四个对象（`playlist`/`resolved_url`/`resolve_result`/`coordinator_result`）写在生成器的具名退出清单里，清单留下契约已不声明的对象即由契约校验报红——「声明了但没生成」在两个平台都不会再悄悄发生
...
## 契约编解码往返（P0 — 合并门槛，Windows 生成物，#365）

- 测试文件：`windows/tests/ContractRoundTripTests.cpp`。用例里不写对象名也不写字段名：对象经生成物的 `generated::ForEachContractObject`
  遍历，字段经每个模型的 `generated::ForEachField` 逐个赋值；契约文件 `contracts/ffi-contract.json` 在运行时读入（路径由构建注入），
  作为「声明了哪些对象、哪些字段」的对照。契约新增对象或字段，重新生成后自动纳入。两个遍历模板只供覆盖全部对象的使用方（目前是本组用例）调用。
- 取值：字符串按字段名各不相同（含非 ASCII）、整数与浮点逐字段递增（`i64` 超出 32 位、浮点带小数）、布尔取真、映射两项、对象字段递归、
  对象列表取一个递归填好的元素（空列表经任何编码器都能往返，证明不了元素走的是契约自己的编解码，#367）——
  都不是字段缺省时解码得到的值，丢字段或两个同类型字段互换都不可能碰巧往返成功。

| 编号 | 行为 | 断言 | 状态 |
|---|---|---|---|
| CR-01 | 契约对象与生成的编解码一一对应 | 契约里除版本、说明与枚举表外的每个条目都在 `ForEachContractObject` 里，反之亦然 | 新测 |
| CR-02 | 全字段有值的往返 | 逐字段构造对象（可选字段全部有值，赋值覆盖契约声明的每个字段）；编码结果与按同一取值写出的预期 JSON 逐字一致（数字类型也不能变）；再解码与原对象逐字段相等（模型为默认 `operator==`） | 新测；编码器去掉一个可选字段、或把一个字段写成另一个字段的值，均立即红 |
...
## 契约编解码往返：可选字段无值（P1，Windows 生成物，#366）

同一测试文件与遍历方式（见上方 CR 段）。对象只设必选字段，可选字段留空；编码器对「缺省」与「显式空值」的处理不同，这是最容易静默出错的地方，逐一锁定。

| 编号 | 行为 | 断言 | 状态 |
|---|---|---|---|
| CR-03 | 缺省的可选字段往返后仍缺省 | 编码结果里没有任何可选字段的键（不写 null、空串或 0），与只含必选字段的预期 JSON 逐字一致；再解码与原对象相等（仍为空） | 新测；编码器把空的 `year` 写成 null、或解码器把缺省的 `year` 读成 0，即红 |
| CR-04 | 显式空值往返后只会变成缺省 | 每个可选字段写成 `null` 的载荷，解码结果与缺省时相同；再编码这些键缺席，不回写 null 也不变成默认值；至少置空过一个字段，不会空跑 | 新测；同上两种缺陷即红 |
| CR-05 | 整型可选字段的缺省形态单独断言 | 模型里的整型可选成员恰为契约声明的 `i32?`/`i64?` 字段；缺省与 `null` 都解码为空（不是 0）且编码时缺席；显式 0 是一个值：编码写出 0、解码读回 0，不被当成缺省。契约里至少要有一个整型可选字段，用例不会空跑 | 新测；编码器丢掉显式 0，只有本条红 |
