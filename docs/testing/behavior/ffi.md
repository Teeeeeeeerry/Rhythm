...
| FF-25 | 消息规格导出（#227/#229） | `rhythm_message_playback_failure` / `rhythm_message_resolve_failure(kind, detail, language)` 返回 `{"segments":[…]}`：键段带 `key`/`params`，字面量段带 `text`；空指针与未知分类 → 回退分支（泛化标题 / 引擎原文） |
