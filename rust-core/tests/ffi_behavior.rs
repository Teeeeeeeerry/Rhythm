...
fn open_lib(dir: &std::path::Path) -> *mut RhythmLibrary {
    ...
    let lib = rhythm_library_open(c(db.to_str().unwrap()).as_ptr());
    assert!(!lib.is_null());
    lib
}
...
fn ff01_open_close_roundtrip() {
    ...
    rhythm_library_close(std::ptr::null_mut()); // must be safe
}

#[test]
fn ff02_open_failure_returns_null() {
    let dir = tempfile::tempdir().unwrap();
    let lib = rhythm_library_open(c(dir.path().to_str().unwrap()).as_ptr());
...
fn ff03_import_directory_counts_and_errors() {
    ...
    let lib = open_lib(dir.path());
    let count = rhythm_library_import(lib, c(music.to_str().unwrap()).as_ptr());
    assert_eq!(count, 2);

    // Errors → -1: null pointer and non-directory path.
    assert_eq!(rhythm_library_import(std::ptr::null_mut(), c("x").as_ptr()), -1);
    let file = music.join("a.wav");
    assert_eq!(rhythm_library_import(lib, c(file.to_str().unwrap()).as_ptr()), -1);
    rhythm_library_close(lib);
}
...
fn ff04_import_file_unsupported_returns_zero() {
    ...
    let lib = open_lib(dir.path());
    let ret = rhythm_library_import_file(lib, c(txt.to_str().unwrap()).as_ptr());
...
fn ff04_import_file_success_and_errors() {
    ...
    let lib = open_lib(dir.path());
    assert_eq!(rhythm_library_import_file(lib, c(wav.to_str().unwrap()).as_ptr()), 1);
    assert_eq!(rhythm_library_import_file(lib, c(missing.to_str().unwrap()).as_ptr()), -1);
    assert_eq!(rhythm_library_import_file(std::ptr::null_mut(), c("x").as_ptr()), -1);
    rhythm_library_close(lib);
}
...
fn ff05_string_memory_contract() {
    ...
    let empty = rhythm_library_get_all_tracks(lib);
    ...
    // Null pointer → null.
    assert!(rhythm_library_get_all_tracks(std::ptr::null_mut()).is_null());
    rhythm_library_close(lib);
}
...
fn ff06_add_track_json_roundtrip() {
    ...
    let saved = rhythm_library_add_track(lib, c(&json).as_ptr());
    assert!(!saved.is_null());
    unsafe {
        let saved_str = take(saved);
    ...
    assert!(rhythm_library_add_track(lib, c("not json{").as_ptr()).is_null());
    rhythm_library_close(lib);
}
...
fn ff07_player_create_destroy() {
    ...
    rhythm_player_destroy(std::ptr::null_mut()); // must be safe
}
...
fn ff08_state_code_mapping() {
    ...
    let player = rhythm_player_create();
    assert_eq!(rhythm_player_get_state(player), 0, "fresh → Stopped");
    rhythm_player_play_file(player, c(wav.to_str().unwrap()).as_ptr());
    std::thread::sleep(std::time::Duration::from_millis(150));
    assert_eq!(rhythm_player_get_state(player), 1, "playing → 1");
    rhythm_player_pause(player);
    assert_eq!(rhythm_player_get_state(player), 2, "paused → 2");
    rhythm_player_stop(player);
    assert_eq!(rhythm_player_get_state(player), 0, "stopped → 0");
    assert_eq!(rhythm_player_get_state(std::ptr::null_mut()), -1, "null → -1");
    rhythm_player_destroy(player);
    ...
    rhythm_player_play_file(player2, c(short.to_str().unwrap()).as_ptr());
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(6);
    while rhythm_player_get_state(player2) != 5 && std::time::Instant::now() < deadline {
        std::thread::sleep(std::time::Duration::from_millis(50));
    }
    assert_eq!(rhythm_player_get_state(player2), 5, "finished → 5");
    rhythm_player_destroy(player2);
}
...
fn ff09_player_error_message() {
    ...
    let player = rhythm_player_create();
    assert!(rhythm_player_error(player).is_null(), "fresh player has no error");

    rhythm_player_play_file(player, c(bad.to_str().unwrap()).as_ptr());
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(5);
    while rhythm_player_get_state(player) != 4 && std::time::Instant::now() < deadline {
        std::thread::sleep(std::time::Duration::from_millis(50));
    }
    assert_eq!(rhythm_player_get_state(player), 4, "corrupted file → Error state");
    unsafe {
...
fn ff10_seek_validation() {
    ...
    assert_eq!(rhythm_player_seek(std::ptr::null_mut(), 1.0), -1, "null → -1");

    let player = rhythm_player_create();
    assert_eq!(rhythm_player_seek(player, -1.0), -1, "negative → -1");

    rhythm_player_play_file(player, c(wav.to_str().unwrap()).as_ptr());
    std::thread::sleep(std::time::Duration::from_millis(150));
    assert_eq!(rhythm_player_seek(player, 99.0), -1, "out of range → -1");
    assert_eq!(rhythm_player_seek(player, 0.1), 0, "in range → 0");
    rhythm_player_destroy(player);
}
...
fn ff11_queue_full_roundtrip() {
    ...
    let q = rhythm_queue_create(c(&queue_json).as_ptr());
    assert!(!q.is_null());
    ...
    assert_eq!(rhythm_queue_has_next(q), 1);
    assert_eq!(rhythm_queue_has_previous(q), 0);
    unsafe {
    ...
    assert_eq!(rhythm_queue_has_next(q), 0, "sequential queue exhausts");
    assert!(rhythm_queue_next(q).is_null(), "exhausted → null");
    ...
    assert_eq!(rhythm_queue_jump_to(q, 1), 0);
    unsafe {
        let cur = take(rhythm_queue_current(q));
        assert!(cur.contains("\"One\""));
    }
    assert_eq!(rhythm_queue_jump_to(q, 2), 0);
    unsafe {
        let cur = take(rhythm_queue_current(q));
        assert!(cur.contains("\"Two\""));
    }
    assert_eq!(rhythm_queue_jump_to(q, 999), -1, "unknown id → -1");

    // SingleLoop: next() stays at the current track.
    rhythm_queue_set_mode(q, 2);
    unsafe {
        let nxt = take(rhythm_queue_next(q));
        assert!(nxt.contains("\"Two\""));
    }
    ...
    rhythm_queue_replace(q, c(&queue_json).as_ptr());
    unsafe {
        let cur = take(rhythm_queue_current(q));
        assert!(cur.contains("\"One\""));
    }
    ...
    rhythm_queue_destroy(std::ptr::null_mut());
}
...
fn ff12_resolve_and_last_error() {
    ...
    let failed = rhythm_resolve_url(c("not a url").as_ptr());
    ...
    let resolved = rhythm_resolve_url(c("https://example.com/ffi-tone.mp3").as_ptr());
    unsafe {
...
fn ff13_classify_url() {
    ...
    let bad = rhythm_classify_url(c("plain text").as_ptr());
...
fn ff14_m3u8_export_import() {
    ...
    assert_eq!(rhythm_export_m3u8(c(out.to_str().unwrap()).as_ptr(), c(&tracks).as_ptr()), 0);
    assert!(out.exists());
    assert_eq!(rhythm_export_m3u8(c(out.to_str().unwrap()).as_ptr(), c("bad{").as_ptr()), -1);

    let imported = rhythm_import_m3u8(c(out.to_str().unwrap()).as_ptr());
    unsafe {
        let json = take(imported);
        assert!(json.contains("Export Me"));
    }
    assert!(rhythm_import_m3u8(c("/nonexistent/nope.m3u8").as_ptr()).is_null());
}
...
fn ff16_metadata_ffi() {
    ...
    assert!(rhythm_metadata_extract(c("/nonexistent/x.mp3").as_ptr()).is_null());
    ...
    assert!(rhythm_metadata_scan(c(tagged.to_str().unwrap()).as_ptr()).is_null(), "file is not a dir");
    ...
    let art = rhythm_metadata_extract_artwork(
        c(art_wav.to_str().unwrap()).as_ptr(),
        c(cache.to_str().unwrap()).as_ptr(),
    );
    assert!(!art.is_null());
    unsafe { let _ = take(art); }
    let plain = rhythm_metadata_extract_artwork(
...
fn ff17_empty_path_inputs_do_not_crash() {
    ...
    let lib = rhythm_library_open(c("").as_ptr());
    if !lib.is_null() {
        rhythm_library_close(lib);
    }
    assert_eq!(rhythm_library_import(std::ptr::null_mut(), c("").as_ptr()), -1);
}

#[test]
fn ff18_queue_create_invalid_json_is_null() {
    assert!(rhythm_queue_create(c("not json").as_ptr()).is_null());
}

#[test]
fn ff19_queue_replace_invalid_json_keeps_queue() {
    let t1 = track_json(1, "Keep Me", "/music/k.mp3");
    let q = rhythm_queue_create(c(&format!("[{t1}]")).as_ptr());

    rhythm_queue_replace(q, c("bad json{").as_ptr());
    unsafe {
        let cur = take(rhythm_queue_current(q));
...
fn ff20_null_pointer_safe_defaults() {
    ...
    rhythm_library_close(null_lib);
    assert_eq!(rhythm_library_import(null_lib, c("x").as_ptr()), -1);
    assert_eq!(rhythm_library_import_file(null_lib, c("x").as_ptr()), -1);
    assert!(rhythm_library_get_all_tracks(null_lib).is_null());
    assert!(rhythm_library_search(null_lib, c("x").as_ptr()).is_null());
    assert!(rhythm_library_add_track(null_lib, c("{}").as_ptr()).is_null());
    assert_eq!(rhythm_library_remove_track(null_lib, 1), -1);
    assert!(rhythm_library_verify_files(null_lib).is_null());
    assert!(rhythm_library_get_playlists(null_lib).is_null());
    assert_eq!(rhythm_library_create_playlist(null_lib, c("n").as_ptr(), c("d").as_ptr()), -1);
    assert_eq!(rhythm_library_playlist_add(null_lib, 1, 2), -1);
    assert_eq!(rhythm_library_playlist_remove(null_lib, 1, 2), -1);
    assert_eq!(rhythm_library_delete_playlist(null_lib, 1), -1);
    assert_eq!(rhythm_library_record_play(null_lib, 1), -1);

    rhythm_player_destroy(null_player);
    assert_eq!(rhythm_player_play_file(null_player, c("x").as_ptr()), -1);
    assert_eq!(rhythm_player_play_url(null_player, c("x").as_ptr()), -1);
    rhythm_player_pause(null_player);
    rhythm_player_resume(null_player);
    rhythm_player_stop(null_player);
    rhythm_player_set_volume(null_player, 0.5);
    assert_eq!(rhythm_player_get_volume(null_player), 0.0);
    assert_eq!(rhythm_player_seek(null_player, 1.0), -1);
    assert_eq!(rhythm_player_get_position(null_player), 0.0);
    assert_eq!(rhythm_player_get_duration(null_player), 0.0);
    assert_eq!(rhythm_player_get_state(null_player), -1);
    assert!(rhythm_player_error(null_player).is_null());

    rhythm_queue_destroy(null_queue);
    assert!(rhythm_queue_current(null_queue).is_null());
    assert!(rhythm_queue_next(null_queue).is_null());
    assert!(rhythm_queue_previous(null_queue).is_null());
    rhythm_queue_set_mode(null_queue, 1);
    assert_eq!(rhythm_queue_jump_to(null_queue, 1), -1);
    rhythm_queue_replace(null_queue, c("[]").as_ptr());
    assert_eq!(rhythm_queue_has_next(null_queue), 0);
    assert_eq!(rhythm_queue_has_previous(null_queue), 0);

    assert!(rhythm_metadata_extract(std::ptr::null_mut()).is_null());
    assert!(rhythm_metadata_scan(std::ptr::null_mut()).is_null());
    assert!(rhythm_metadata_extract_artwork(std::ptr::null_mut(), std::ptr::null_mut()).is_null());
    assert!(rhythm_resolve_url(std::ptr::null_mut()).is_null());
    assert!(rhythm_classify_url(std::ptr::null_mut()).is_null());
}
...
fn ff21_large_json_roundtrip() {
    ...
    let q = rhythm_queue_create(c(&json).as_ptr());
    assert!(!q.is_null());
    unsafe {
        let cur = take(rhythm_queue_current(q));
