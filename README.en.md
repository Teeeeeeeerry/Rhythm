...
Initial development is complete. Current version: **v0.5.210 "Motif"** (kept in sync with `Cargo.toml`; bump this line on every release).
...
```bash
python3 scripts/tasks.py test     # both platforms first run the same static analysis: ten L0 checks + zero-emoji + two self-test suites
