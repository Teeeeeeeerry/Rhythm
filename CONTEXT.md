...
scripts/            build-macos.sh / build-rust-macos.sh / build-windows.* / check-no-emoji.sh
```
...
- **L10n**：全部文案走 `L10n` 枚举（macOS `Models/L10n.swift` 的 `isChinese` 三元、Windows `L10n.h`），
  视图里既不写死字符串也不就地写 inline 三元——#145 曾一次性收敛 20+ 处违规
